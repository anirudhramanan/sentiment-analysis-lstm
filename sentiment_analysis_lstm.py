...
# get_ipython().magic(u"run -i 'scripts/data_download.py'")
...
# get_ipython().magic(u"run -i 'scripts/data_prep.py'")
...
with graph.as_default():
    ...
    # add the prediction to tensorboard
    tf.summary.histogram('predictions', predictions)
    
    cost = tf.losses.mean_squared_error(labels_, predictions)
    
    # add the cost to tensorboard
    tf.summary.scalar('cost', cost)
    
    # use adam optimizer to minimise the cost
...
with tf.Session(graph=graph) as sess:
    ...
    # create the train write file for tensorboard
    ...
    for e in range(epochs):
        ...
        for ii, (x, y) in enumerate(get_batches(train_x, train_y, batch_size), 1):  
            feed_dict={inputs_:x, labels_:y[:, None], keep_prob:0.8}
            summary, a, o, _ = sess.run([merged, accuracy, cost, optimizer], feed_dict=feed_dict)
            
            # add the summary to tensorboard
            train_writer.add_summary(summary, iteration)
            
            # print the accuracy
            print('Batch: {}'.format(ii), ' | Loss: {}'.format(o), 
                 ' | Accuracy: {}'.format(a))
            
            # append the accuracy, used to calculate the mean at the end
